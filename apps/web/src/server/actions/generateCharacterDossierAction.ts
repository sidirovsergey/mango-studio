...
export async function generateCharacterDossierAction(rawInput: unknown): Promise<
  { ok: true } | { ok: false; error: string; error_code?: string }
> {
  ...
  const style = (character.config_overrides?.style ?? project.style ?? '3d_pixar') as
    | '3d_pixar' | '2d_drawn' | 'clay_art'

  const prompt = input.custom_prompt
  ...
  const quality = (character.config_overrides?.quality ?? (tier === 'premium' ? '1080p' : '720p')) as '720p' | '1080p' | '2k'
  ...
  try {
    ...
    const { error: updateErr } = await sb
      .from('projects')
      .update({ script: { ...script, characters: newCharacters } as never, updated_at: new Date().toISOString() })
      .eq('id', input.project_id)
      .eq('user_id', user.id)
