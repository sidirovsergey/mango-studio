...
export async function refineCharacterAction(rawInput: unknown): Promise<
...
> {
  ...
  const modelId = process.env.LLM_MODEL_REFINE ?? process.env.LLM_MODEL_DEFAULT ?? 'x-ai/grok-4.1-fast'

  const { object } = await generateObject({
    model: openrouter(modelId),
    schema: RefineOutputSchema,
    system: 'Ты обновляешь карточку персонажа. Возвращай обновлённые description (1-2 предложения), appearance (структурно), personality (если изменилась).',
