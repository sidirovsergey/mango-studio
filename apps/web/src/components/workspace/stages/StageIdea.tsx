...
export function StageIdea({ project }: Props) {
  const formatLabel =
    project.format === '9:16' ? 'Вертикаль' : project.format === '16:9' ? 'Горизонталь' : 'Квадрат';
  const styleLabel =
    project.style === '3d_pixar'
      ? '3D Pixar'
      : project.style === '2d_drawn'
        ? '2D рисованный'
        : 'Клей-арт';
  ...
  return (
    <section className="stage" data-stage id="ideaStage">
      ...
      <div className="idea-summary">
        <button type="button" className="meta-tile" data-edit="duration">
      ...
      <p className="stage-idea-text" style={{ marginTop: 16, color: 'var(--ink-500)' }}>
        {project.idea}
      </p>
    </section>
  );
