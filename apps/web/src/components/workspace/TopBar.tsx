  ...
  return (
    <div className="topbar">
      <div className="brand">
        ...
        <span className="brand-name">Mango<span>Studio</span></span>
      </div>
      <div className="topbar-right">
        <div className="seg" role="tablist" aria-label="Aspect" id="aspectSeg">
          {(['9:16', '16:9', '1:1'] as const).map((a) => (
            <button key={a} className={format === a ? 'active' : ''} data-aspect={a}>
              {a}
            </button>
          ))}
        </div>
        <label className="auto-mode-toggle" style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}>
          <input
            type="checkbox"
            checked={autoMode}
          ...
          />
          <span style={{ fontSize: '13px', color: autoMode ? 'var(--mango-500)' : 'var(--ink-400)' }}>
            Авто-режим
          </span>
        </label>
        <div className="credits"><span className="dot" />~12 480 кр</div>
        <button className="cta" id="publishBtn" disabled>
