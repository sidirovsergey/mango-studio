...
- refine_script(instruction): полностью переписать сценарий по инструкции пользователя ("сделай веселее", "добавь героиню-кошку", "переделай развязку"). Используется И для добавления новой сцены (через инструкцию "добавь сцену про X").
...
- "добавь сцену про X" → refine_script(instruction: "добавь сцену про X в конец/после такой-то")
