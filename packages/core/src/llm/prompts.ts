...
- generate_character(character_id): нарисовать ВИЗУАЛЬНОЕ ДОЗЬЁ существующего персонажа через fal.ai (~10-20 сек). character_id бери из блока АКТИВНЫЕ ПЕРСОНАЖИ. ВАЖНО: если has_dossier=true (картинка уже есть) — НЕ вызывай tool сразу. Сначала ответь текстом «У X уже есть дозьё, перегенерация заменит существующее. Подтверди?» — и дождись подтверждения юзера.
