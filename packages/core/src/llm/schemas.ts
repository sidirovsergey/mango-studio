...
export const SceneSchema = z.object({
  ...
  description: z.string().min(1).describe('Описание сцены, что в ней происходит, для media-генерации'),
...
export const ScriptGenSchema = z.object({
  ...
  scenes: z.array(SceneSchema).min(2).max(8).describe('2-8 сцен, в сумме укладывающихся в target_duration_sec'),
  characters: z.array(CharacterSchema).min(1).max(5).describe('Главные персонажи, фигурирующие в сценах'),
});
