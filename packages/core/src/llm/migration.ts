...
import type { MasterClipVersion, SceneAssetVersion } from '../media/scene-types';
...
function makeVersion(legacyAsset: any): SceneAssetVersion {
  return {
    ...
    source:
      legacyAsset.source === 'ai_text2img' || legacyAsset.source === 'ai_img2img_continuity'
        ? 'auto_continuity'
        : legacyAsset.source === 'user_upload'
          ? 'user_upload'
          : 'manual_text2img',
  };
}
...
export function upgradeScene(legacy: LegacyScene): Scene {
  const ffVersion =
    legacy.first_frame && legacy.first_frame !== null ? makeVersion(legacy.first_frame) : null;
...
export function upgradeScript(legacy: LegacyScript): Script {
  const masterVersion: MasterClipVersion | null = legacy.master_clip
    ? {
        version_id: randomUUID(),
        storage: legacy.master_clip.storage,
        generated_at: legacy.master_clip.generated_at,
        cost_usd: legacy.master_clip.cost_usd ?? null,
        composed_from_scene_versions: (legacy.master_clip.scene_ids_snapshot ?? []).map(
          (sid: string) => ({
            scene_id: sid,
            video_version_id: 'legacy',
...
export function downgradeScript(script: Script): unknown {
  ...
  const downScenes = script.scenes.map((s) => {
    const ff = s.first_frame_active_version_id
      ? (s.first_frame_versions.find((v) => v.version_id === s.first_frame_active_version_id) ??
        null)
      : null;
    ...
    return {
      ...
      first_frame_source: s.first_frame_source,
      first_frame: ff
        ? {
            storage: ff.storage,
            model: ff.model,
      ...
      video: vid
        ? {
            storage: vid.storage,
            model: vid.model,
            generated_at: vid.generated_at,
            has_native_audio: vid.has_native_audio ?? false,
            source: 'ai_img2vid',
            duration_sec: s.duration_sec,
          }
        : null,
      last_frame: s.last_frame,
      final_clip: s.final_clip,
    };
  });
  const masterActive = script.master_clip_active_version_id
    ? (script.master_clip_versions.find(
        (v) => v.version_id === script.master_clip_active_version_id,
      ) ?? null)
    : null;
  return {
    title: script.title,
    ...
    master_clip: masterActive
      ? {
          storage: masterActive.storage,
          generated_at: masterActive.generated_at,
