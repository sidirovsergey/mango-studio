...
describe('SupabaseStorage', () => {
  it('persist: fetches fal URL и uploads с правильным path', async () => {
    ...
    const s = new SupabaseStorage(client as never, 'character-dossiers', { fetchImpl: fetchMock as typeof fetch })
  ...
  it('getDisplayUrl для fal_passthrough = pass-through', async () => {
    const client = makeMockClient()
    const s = new SupabaseStorage(client as never, 'character-dossiers')
    const url = await s.getDisplayUrl({ kind: 'fal_passthrough', url: 'https://v3.fal.media/x.png' })
